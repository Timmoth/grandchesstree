...
{
    public static class ExpandKernelWhite
    {
            ...
            GpuAttackTable gpuAttackTables, TotalStatsLayerBuffers stats)
        {
            ...
            MoveExtensionsWhite.ApplyMoves(move3, ref pawn, ref knight, ref bishop, ref rook, ref queen, ref white, ref black, ref whiteKingPos, ref blackKingPos, ref castleRights, ref enPassantFile);

            var occupancy = white | black;
            var diagonalSliders = bishop | queen;
            var straightSliders = rook | queen;

            var checkers = gpuAttackTables.PextBishopAttacks(occupancy, whiteKingPos) & black & diagonalSliders |
               gpuAttackTables.PextRookAttacks(occupancy, whiteKingPos) & black & straightSliders |
               gpuAttackTables.KnightAttackTable[whiteKingPos] & black & knight |
               gpuAttackTables.WhitePawnAttackTable[whiteKingPos] & black & pawn;

            var numCheckers = IntrinsicMath.PopCount(checkers);

            var moveMask = numCheckers == 0 ? 0xFFFFFFFFFFFFFFFF : checkers | gpuAttackTables.LineBitBoardsInclusive[whiteKingPos * 64 + IntrinsicMath.TrailingZeroCount(checkers)];
            var pinMask =
                gpuAttackTables.DetectPinsDiagonal(whiteKingPos, black & diagonalSliders, occupancy) |
                gpuAttackTables.DetectPinsStraight(whiteKingPos, black & straightSliders, occupancy);

            var attackedSquares = 0ul;

            var occupancyMinusKing = occupancy ^ 1ul << whiteKingPos;
            ...
            attackedSquares |= gpuAttackTables.KingAttackTable[blackKingPos];

            // Generate all possible moves and write them to the output

            var kingMoves = gpuAttackTables.KingAttackTable[whiteKingPos] & ~attackedSquares;
