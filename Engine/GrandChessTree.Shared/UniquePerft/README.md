...
The Rust merger lives under `Unique-Positions/src/wave_merge.rs` (uses
`memmap2` + `rayon`).

## Running it

```text
# UCI shim (Engine/GrandChessTree.Engine)
...
WAVE_MERGE_DELETE_AFTER=1 \
  Unique-Positions/target/release/wave_merge \
  <spill_dir> <output_wave_dir>
```

See `Engine/GrandChessTree.Engine/Program.cs` for the full command
