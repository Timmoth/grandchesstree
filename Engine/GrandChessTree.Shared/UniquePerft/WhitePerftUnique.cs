...
public bool CanWhitePawnEnpassant()
{
    ...
    // Leaf emit for a position where White is to move. Called from Black
    // piece accumulators at depth=1, and from AccumulateWhiteMovesUnique at
    // depth=0. AggressiveInlining lets the JIT fold this into the call site
    // when visible — saving the depth==0 branch and method-call dispatch on
    // the hot path.
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public unsafe void EmitLeafWhite()
    {
        var hash = Hash;
        bool epAvailable = false;
        if (EnPassantFile < 8)
        {
            if (CanWhitePawnEnpassant()) epAvailable = true;
        ...
        if (posSink == null)
        {
            EmitLeafWhiteCold(hash, epAvailable);
            return;
        }
        ...
        if (mtSuppress != null)
        {
            var h2s = SecondaryHash.Compute(ref this, whiteToMove: true, epAvailable);
        ...
        BoardStateSerialization.WriteToSpan(ref snap, buf, whiteToMove: true);
        BinaryPrimitives.WriteUInt64LittleEndian(buf.Slice(26), PerftUnique.NextDfsCounter());
        posSink.Record(buf, hash);
    }

    [MethodImpl(MethodImplOptions.NoInlining)]
    private unsafe void EmitLeafWhiteCold(ulong hash, bool epAvailable)
    {
        ...
        if (sink != null)
        {
            var h2 = SecondaryHash.Compute(ref this, whiteToMove: true, epAvailable);
        ...
        if (rankIndex.IsSeventhRank())
        {
            ...
            while (validMoves != 0)
            {
                ...
                newBoard.WhitePawn_Capture_KnightPromotion(index, toSquare);
                 if (depth == 1) newBoard.EmitLeafBlack(); else newBoard.AccumulateBlackMovesUnique(depth - 1);
                newBoard = Unsafe.As<Board, Board>(ref this);
