...
// Record layout (DFS-sort-compression experiment): 34 bytes per record:
//   bytes 0..26  — canonical board position (BoardStateSerialization)
//   bytes 26..34 — u64 LE DFS-emission sortKey, written by the engine leaf.
//
// Why the sortKey rides through the merger: per-bucket sort+dedup runs on
// position bytes (0..26) only. The sortKey is preserved on the slice (min
// sortKey on equal-position runs by virtue of lexicographic [position|sortKey]
// ordering). Phase-3 `wave_global_merge` then k-way merges across all buckets
// keyed on sortKey, producing a chess-adjacency-ordered global stream that
// `xz --delta` can compress dramatically better than the hash-bucket order.
const RECORD: usize = 34;
const POSITION: usize = 26;
...
struct Progress {
    ...
    // Sum of per-bucket sort wall-time across all processed buckets. For the
    // parallel-radix-sort experiment — compare wall-clock sort cost between
    // WAVE_MERGE_SORT=seq (default) and =par.
    sort_ms_total: AtomicU64,
    stop: AtomicBool,
...
fn main() {
    ...
    // If only one bucket fits in RAM, take the explicitly-sequential code path
    // below — par_iter() on a default rayon pool would still try to run num_cpus
    // buckets concurrently and OOM us on a big-bucket ply.
    let use_sequential = sequential || parallel_jobs <= 1;
    if !use_sequential {
    ...
    println!(
        ...
        if use_sequential { "sequential".to_string() } else { format!("parallel x{}", parallel_jobs) },
    ...
    let results: Vec<(String, u64, u64)> = if use_sequential {
        to_process
            .iter()
            .map(|path| {
    ...
    let grand_sort_ms = progress.sort_ms_total.load(Ordering::Relaxed);
    let sort_mode = std::env::var("WAVE_MERGE_SORT").unwrap_or_else(|_| "seq".to_string());

    println!();
    println!("total records seen    : {grand_seen}");
    println!("total unique positions: {grand_uniq}");
    println!("sort wall (sum)       : {grand_sort_ms} ms  (mode={sort_mode})");
    println!(
        "dedup ratio           : {:.2}x",
...
fn process_bucket(
    ...
    // Lexicographic sort over [u8; 34] = position bytes first, sortKey tiebreak.
    // Equal-position records group together, smallest sortKey first within the
    // group → dedup_sorted_in_place (keep first) yields the min-sortKey
    // representative per position, exactly what Phase 3's global merge wants.
    //
    // WAVE_MERGE_SORT env var selects the algorithm:
    //   unset / "seq" / "sort_unstable" — std `sort_unstable()` (default,
    //     single-threaded inside a bucket; merger's outer rayon pool runs
    //     buckets in parallel).
    //   "par" / "par_sort_unstable" — rayon `par_sort_unstable()`, which
    //     parallelizes the comparison sort within a single bucket. Useful
    //     when outer parallelism is capped low by RAM budget at high K.
    let sort_t0 = Instant::now();
    match std::env::var("WAVE_MERGE_SORT").as_deref() {
        Ok("par") | Ok("par_sort_unstable") => {
    ...
    let sort_elapsed_ms = sort_t0.elapsed().as_millis() as u64;
    progress.sort_ms_total.fetch_add(sort_elapsed_ms, Ordering::Relaxed);
    let before = recs.len() as u64;
    let unique_len = dedup_sorted_in_place(recs);
    let after = unique_len as u64;

    // Slice is sorted by position; each record still carries its emission
    // sortKey at bytes 26..34. Phase 3 will re-sort across buckets by sortKey.
    let unique_slice = &mut recs[..unique_len];
    ...
    let unique_bytes = unsafe {
        std::slice::from_raw_parts(unique_slice.as_ptr() as *const u8, unique_len * RECORD)
    };
    let compress = std::env::var("WAVE_MERGE_COMPRESS").ok().is_some();
    if compress {
        // Pipe through `xz -c -T N --delta=dist=26 --lzma2=preset=1,dict=4MiB`.
        // Each parallel bucket spawns its own xz; with `-T 1` we observed
        // xz-compression-throughput as the merger's bottleneck (~30-50 MB/s
        // per bucket). Default WAVE_MERGE_XZ_THREADS=4 lets xz block-parallelise
        // each bucket's input — costs ~5-10 % compression ratio (block-
        // boundary dictionary resets) but multiplies throughput 3-4×.
        let xz_threads = std::env::var("WAVE_MERGE_XZ_THREADS")
            .ok()
            .and_then(|s| s.parse::<u32>().ok())
            .filter(|n| *n > 0)
            .unwrap_or(4);
        // WAVE_MERGE_XZ_PRESET (default 1): LZMA2 compression level 0-9.
        // 0 ≈ 2× faster, ~10 % bigger output vs 1; 1 is the historical default.
        // For mid-pipeline waves (ply 11+), preset 0 cuts merge wall time
        // substantially with only a marginal disk increase.
        let xz_preset = std::env::var("WAVE_MERGE_XZ_PRESET")
            .ok()
            .and_then(|s| s.parse::<u32>().ok())
            .filter(|n| *n <= 9)
            .unwrap_or(1);
        let out_file = fs::File::create(&tmp_path).expect("create tmp");
        let xz_threads_str = xz_threads.to_string();
        let lzma2_arg = format!("--lzma2=preset={},dict=4MiB", xz_preset);
        // --delta=dist must match RECORD so adjacent records align under the
        // delta filter. With 34-byte records (position+sortKey), dist=34. The
        // random-looking sortKey tail compresses poorly under delta; for the
        // experiment we run with WAVE_MERGE_COMPRESS unset (default off) and
        // let Phase 3's global merge produce the artifact we measure.
        let mut child = Command::new("xz")
            .args(["-c", "-T", xz_threads_str.as_str(), "--delta=dist=34", &lzma2_arg])
            .stdin(Stdio::piped())
            .stdout(Stdio::from(out_file))
            .spawn()
            .expect("spawn xz (is xz installed?)");
        {
            let stdin = child.stdin.take().expect("xz stdin");
            let mut bw = std::io::BufWriter::with_capacity(1 << 20, stdin);
            bw.write_all(unique_bytes).expect("write to xz stdin");
            bw.flush().expect("flush to xz stdin");
            // dropping bw drops the stdin → signals EOF to xz so it finalizes.
        }
        let status = child.wait().expect("wait for xz");
        if !status.success() {
            panic!("xz exited with {} on bucket {}", status, name);
        }
    } else {
...
// Dedup a sorted slice in place by position bytes only (the 26-byte prefix).
// Equal-position records are adjacent thanks to lexicographic sort over the
// 34-byte record; we keep the first occurrence (= min sortKey) and drop the
// rest. The retained record's sortKey tail (bytes 26..34) survives untouched
// for Phase 3.
fn dedup_sorted_in_place(s: &mut [[u8; RECORD]]) -> usize {
    ...
    let mut write = 1;
    for read in 1..s.len() {
        if s[read][..POSITION] != s[write - 1][..POSITION] {
            if read != write {
                s[write] = s[read];
            }
            write += 1;
