 ...
 *      src/articles/MoveGen/MoveGen.Wasm/publish-to-webapp.sh, which is too slow to
...
const NAV_SECTIONS = ["perft-results", "tools", "move-generator", "cpu-leaderboard", "gpu-leaderboard"];
...
function detectActiveSection(html) {
  ...
  if (/href="(?:\.\.\/)?perft-results\.html"[^>]*class="text-slate-900"/.test(header)) return "perft-results";
...
// Leaderboard/perft_war.py aggregate) and rebuild — the cards, table, badge,
...
function leaderboardVars() {
  ...
  // "run in progress, no value yet" (Leaderboard writes nulls as placeholders).
