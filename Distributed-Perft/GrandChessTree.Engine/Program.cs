...
namespace GrandChessTree.Engine
{
    internal class Program
    {
        ...
        {
            if (commandParts.Length < 5 || commandParts.Length > 7 ||
                !int.TryParse(commandParts[3], out var numBuckets) ||
                !int.TryParse(commandParts[4], out var threadCount))
            {
                Console.WriteLine("Invalid command format: 'wave_expand:<in_dir>:<out_dir>:<buckets>:<threads>[:<log2_memtable>[:<mode>]]'.");
            ...
            foreach (var f in allInputFiles)
            {
                ...
                {
                    skipped++;
                    skippedRecords += new FileInfo(f).Length / BucketPositionSpillSink.RecordSize;
                }
                else
                {
                    inputFiles.Add(f);
            ...
            {
                var n = new FileInfo(f).Length / BucketPositionSpillSink.RecordSize;
                totalInputRecords += n;
            ...
            if (mode == "count")
            {
                var sink = new BucketSpillSink(outDir, numBuckets);
                PerftUnique.SpillSink = sink;
                sinkDisposable = sink;
            ...
            {
                var sink = new BucketPositionSpillSink(outDir, numBuckets);
            ...
            Console.WriteLine($"  mode: {mode} ({recBytes}-byte spill records)");
            ...
            for (int i = 0; i < threadCount; i++)
            {
                ...
                threads[idx] = new Thread(() =>
                {
                    ...
                    while (fileQueue.TryDequeue(out var path))
                    {
                        ...
                        using (var fs = new FileStream(path, FileMode.Open, FileAccess.Read, FileShare.Read, 1 << 20, useAsync: false))
                        {
                            long fileLen = fs.Length;
                            long records = fileLen / BucketPositionSpillSink.RecordSize;
                            long doneInFile = 0;
                            while (true)
                            {
                                int got = 0;
                                ...
                                {
                                    int r = fs.Read(buf, got, BucketPositionSpillSink.RecordSize - got);
                                    if (r == 0) break;
                                    got += r;
                                }
