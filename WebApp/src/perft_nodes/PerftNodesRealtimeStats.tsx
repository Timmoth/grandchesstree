...
const PerftNodesRealtimeStats: React.FC<RealtimeStatsProps> = ({ positionId, depth }) => {
    ...
    const fetchLeaderboard = async () => {
      try {
        const resp = await fetch(
          `https://api.grandchesstree.com/api/v2/perft/nodes/${positionId}/${depth}/stats`
        );
        if (!resp.ok) {
...
export default PerftNodesRealtimeStats;
